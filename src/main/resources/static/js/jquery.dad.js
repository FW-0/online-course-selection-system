...
(function ($) {
  ...
  $.fn.dad = function (opts) {
    ...
    $(this).each(function () {
      ...
      let $daddy = $(this);
      let childrenClass = options.childrenClass;
      let cloneClass = options.cloneClass;
      let jQclass = '.' + childrenClass;
      let $target = $daddy.find(options.target);
      let placeholder = options.placeholder;
      let callback = options.callback;
      let dragClass = 'dad-draggable-area';
      let holderClass = 'dads-children-placeholder';
      ...
      $(window).on('mousemove touchmove', function (e) {
        ...
        if (supportsTouch && e.type == 'touchmove') {
          ...
          let mouseTarget = document.elementFromPoint(ev.clientX, ev.clientY);
      ...
      function dadEnd() {
        if (mouse.target != false &&  mouse.clone != false) {
          ...
          let appear = mouse.target;
          let desappear = mouse.clone;
          let holder = mouse.placeholder;
          let bLeft = 0;
          let bTop = 0;
      ...
      function dadUpdate(obj) {
        if (mouse.target !== false && mouse.clone !== false) {
          let $origin = $('<span style="display:none"></span>');
          let $newplace = $('<span style="display:none"></span>');
      ...
      // GRABBING EVENT
      let jq = (options.draggable !== false) ? options.draggable : jQclass;
      ...
      $daddy.on('mousedown touchstart', jq, function (e) {
        ...
        if (mouse.target == false && active == true && (e.which == 1 || e.type == 'touchstart')) {
          ...
          let bLeft = Math.floor(parseFloat($daddy.css('border-left-width')));
          let bTop = Math.floor(parseFloat($daddy.css('border-top-width')));
          let difx = mouse.x - mouse.target.offset().left + $daddy.offset().left + bLeft;
          let dify = mouse.y - mouse.target.offset().top + $daddy.offset().top + bTop;
